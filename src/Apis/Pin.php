...
class Pin extends Api
{
    ...
    {
        if (strlen($old_pin) > 6) {
            $old_pin = $this->encryptPin($this->getFormattedPublicKey($old_pin));
        } elseif ($old_pin == '') {
            $old_pin = '';
        } else {
    ...
    public function getFormattedPublicKey(string $pin): string
    {
        if (strlen($pin) > 6) {
            ...
            $public_base64url = self::getPublicFromEd25519PrivateKey($pin);
    ...
    public function verifyPin(string $pin): array
    {
        if (strlen($pin) > 6) {
            ...
            $body = [
                'pin_base64' => $this->encryptPin(MixinSDKTrait::signWithEd25519(Base64Url::decode($pin), 'TIP:VERIFY:'.$_timestamp)),
